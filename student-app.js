...
function renderAnalysisReport(data) {
    const { documentType, coreSummary, logicFlowchart, structuralComparison, plagiarismSuspicion } = data;
    ...
    // Calculate scores
    const textPlagiarismScore = calculateTextPlagiarismScore(plagiarismSuspicion);
    ...
    if (plagiarismSuspicion && plagiarismSuspicion.length > 0) {
        hasContent = true;
        plagiarismSuspicion.sort((a, b) => b.similarityScore - a.similarityScore).forEach(item => {
            const itemDiv = document.createElement('div');
            itemDiv.className = 'report-item textual';
            itemDiv.innerHTML = `<h4>텍스트 유사성 (${item.similarityScore}%)</h4><p><strong>원본 의심 문장:</strong> "${item.userSentence}"</p><p><strong>출처:</strong> ${item.source}</p>`;
            reportContainer.appendChild(itemDiv);
        });
    }
